...
#endif
/*instance of mailbox is created as a pointer instance - barrier
  global semaphore is created as a pointer - processorAvailable*/
		    ...
		    BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
  ...
  /*A fork() is made to spawn a new child process
    the integer value returned from the fork is assigned to local variable pid*/
  int pid = fork ();
  /*If the integer value returned is 0 then the process is a child
    function flow drops into the following section if the conditional
    statement is satisfied*/
  if (pid == 0)
    {
      /* child is the source which spawns each move on a separate core.  */
      /*For loop to iterate through the total number of moves
        passed through to the function - noOfMoves*/
      for (i = 0; i < noOfMoves; i++)
      {
	/*The process waits for a new processor to become available
	  when the semaphore processorAvailable is incremented
	  greater than 0 the child accepts the semaphore and
	  decrements it by 1*/
	multiprocessor_wait (processorAvailable);
	/*If fork() == 0 returns true, the process is a child*/
	 if (fork () == 0)
	    {
	     /* child must search move i.  */
	      /*alphaBeta search algortithm is called to determine the best move
	       with each loop of the forloop above a different move is passed to
	       this function to be evaluated. The evaluated score is returned and
	       assigned to the local variable- move_score*/
	      move_score = alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore);
	      /*mailbox_send prepares the data to be sent back to the parent process
		via the mailbox - barrier
	        variables move_score,i the move number and the number of positions explored
	        are passed to the function*/
	      mailbox_send (barrier, move_score, i, positionsExplored);
	      /*Process signals that it has completed the task and the
	        processorAvailable semaphore is incremented by 1
	        This processor is now available for another process*/
	      multiprocessor_signal(processorAvailable);
	      /*child process terminates - exit(0)*/
	      exit (0);
	    }
      }
      exit (0);
    }
  /*If the value of pid is not = 0 then it is a parent process
   function flow continues into this code block*/
   else
    {
      ...
      /*Local variables declared to store the values returned from the
        data sent from the child porocess*/
      ...
      for (i=0; i < noOfMoves; i++)
	{
	  ...
	  /*mailbox_rec retrieves the data sent via the mailbox buffer from any
	   one of the child processes
	   move_score, move_index, positionsExplored values assigned to the
	   corresponding local variables*/
	  mailbox_rec (barrier, &move_score, &move_index, &positionsExplored);
	  /*data received is displayed to console*/
	  printf (" ... parent has received a result: move %d has a score of %d after exploring %d positions\n",
		 l[ move_index], move_score, positionsExplored);

	  *totalExplored += positionsExplored;  /* add count to the running total.  */
	  /*If the returned value of move_score is greater than the current best
	   best = move_score. otherwise, code is skipped and best is unmodified*/
	  if (move_score > best)
	    {
	      best = move_score;
	      *move = l[move_index];
